...
from utils.metrics import bbox_iou, wasserstein_loss, inner_siou, inner_ciou
...
class BboxLoss(nn.Module):
    def __init__(self, reg_max, use_dfl=False):
        ...
        self.nwd_weight = 0.5
        self.use_nwd = False
        self.iou_model = ['bbox_iou', 'inner_siou', 'inner_ciou'][0]
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        if self.iou_model == 'bbox_iou':
            iou = eval(self.iou_model)(pred_bboxes_pos, target_bboxes_pos, xywh=False, CIoU=True)
            # iou = eval(self.iou_model)(pred_bboxes_pos, target_bboxes_pos, xywh=False, DIoU=True)
            # iou = eval(self.iou_model)(pred_bboxes_pos, target_bboxes_pos, xywh=False, MDPIoU=True)
            # iou = seval(self.iou_model)(pred_bboxes_pos, target_bboxes_pos, xywh=False, GIoU=True)
        else:
            iou = eval(self.iou_model)(pred_bboxes_pos, target_bboxes_pos, xywh=False)
        loss_iou = 1.0 - iou
        ...
        if self.use_nwd:
            ...
            nwd_loss = ((1.0 - nwd) * bbox_weight).sum() / target_scores_sum
            loss_iou = self.nwd_weight * loss_iou + (1 - self.nwd_weight) * nwd_loss
